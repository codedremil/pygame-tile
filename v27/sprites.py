...
class Player(pg.sprite.Sprite):
    ...
    def hit(self):
        self.damaged = True
        self.damage_alpha = chain(DAMAGE_ALPHA * 2)    # on a besoin d'un itérateur pour next()
    ...
    def update(self):
        ...
        collide_with_walls(self, self.game.breakable_obstacles, 'x')  # part27/7
        self.hit_rect.centery = self.pos.y
        collide_with_walls(self, self.game.walls, 'y')
        collide_with_walls(self, self.game.breakable_obstacles, 'y')  # part27/7 (TESTER)
...
class Obstacle(pg.sprite.Sprite):
    def __init__(self, game, x, y, w, h):
        self._layer = WALL_LAYER # part27/1 !!!
...
# part27/2: copie de la classe Obstacle        
class BreakableObstacle(pg.sprite.Sprite):
    #def __init__(self, game, x, y, w, h):
    def __init__(self, game, type, x, y):  # part27/13
        self._layer = WALL_LAYER
        self.groups = game.all_sprites, game.breakable_obstacles   # part27/13: groupe à part
        pg.sprite.Sprite.__init__(self, self.groups)
        self.game = game
        # part27/13: supprime le bloc
        #self.rect = pg.Rect(x, y, w, h)  
        #self.hit_rect = self.rect
        #self.x = x
        #self.y = y
    ...
    # part27/30: effet vidéo d'un obstacle touché
    def update(self):
        if self.damaged:
            try:
                self.image.fill((250, 250, 250, next(self.damage_alpha)), special_flags=pg.BLEND_RGB_MULT)
            except StopIteration:
                self.damaged = False
    ...
    def broken(self):
        if self.type in ['window_h', 'window_v']:
            ...
            if self.health > 0:
                ...
                self.game.map_img.blit(self.game.broken_door, 
                        (self.rect.centerx - TILESIZE // 2, self.rect.centery - TILESIZE // 2))
...
class Mob(pg.sprite.Sprite):
    ...
    def update(self):
        ...
        if target_dist.length_squared() < DETECT_RADIUS ** 2:
            ...
            #part27/32: effet vidéo quand le zombie est touché
            if self.damaged:
                try:
            ...
            self.hit_rect.centerx = self.pos.x
            collide_with_walls(self, self.game.walls, 'x')
            collide_with_walls(self, self.game.breakable_obstacles, 'x') # part27/16
            self.hit_rect.centery = self.pos.y
            collide_with_walls(self, self.game.walls, 'y')
            collide_with_walls(self, self.game.breakable_obstacles, 'y') # part27/16            
            self.rect.center = self.hit_rect.center
...
class Bullet(pg.sprite.Sprite):
    ...
    def update(self):
        ...
        # # part27/8: ajouter une vitre dans le jeu et tester
        # hits_d = pg.sprite.spritecollide(self, self.game.breakable_obstacles, True)
        # if hits_d:
        #     self.kill()
            
        # part27/20: (met en commentaire le part27/8)
