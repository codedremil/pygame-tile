...
class Player(pg.sprite.Sprite):
    def __init__(self, game, x, y):
        ...
        # part28/10: augment le spread des tirs quand le joueur bouge
        self.moving = False
    ...
    def shoot(self):
        ...
        if now - self.last_shot > WEAPONS[self.weapon]['rate']:
            ...
            for i in range(WEAPONS[self.weapon]['bullet_count']):
                # part28/12: le spread augmente avec le mouvement
    ...
    def update(self):
        ...
        # part28/11: Check if we move (notre vecteur est de la forme vec(x, 0))
        if self.vel.x == 0:
            self.moving = False
        else:
            self.moving = True        
...
class BreakableObstacle(pg.sprite.Sprite):
    ...
    def broken(self):
        if self.type in ['window_h', 'window_v']:
            rot = choice([0, 90, 180, 270])  # part28/1 et ajout de la rotation ci-dessous
            ...
            rot = choice([0, 90, 180, 270])  # part28/1
            if self.health > 0:
                self.game.effects_sounds['hit_door'].play()
            else:
                # part28/2: ajout de la rotation
...
class Mob(pg.sprite.Sprite):
    ...
    def draw_health(self):
        ...
        #part28/9: affiche la barre différemment avec un petit cadre noir
