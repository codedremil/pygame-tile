'''
Quelques améliorations visuelles pour rompre la monotonie (bris de glace...)
Des petits symboles autour des mobiles et du joueur...
'''
import pygame as pg
import sys
from os import path
...
class Game:
    ...
    def load_data(self):
        ...
        for dest_obs in DEST_OBS_IMAGES:
            self.dest_obs_imgs[dest_obs] = pg.image.load(path.join(img_folder, DEST_OBS_IMAGES[dest_obs])).convert_alpha()            
    ...
    def update(self):
        ...
        now = pg.time.get_ticks() # part28/3
        for hit in hits:
            # part28/3: on veut un colldown car si le joueur touche trop longtemps
            ...
            if now - self.player.last_hit > COOLDOWN:
                ...
                # part28/5: en décalant ces lignes le joueur peut passer à travers
