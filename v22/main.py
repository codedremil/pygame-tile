'''
Ajout de l'ecran de fin de jeu et correction de qq bugs
Bug 1 : quand le jeu redemarre, les zombies restent morts et la tache verte est toujours la
On ajoute le nombre de Zombies restants
Bug 2 : si on tire avec le pistolet et que la balle n'a pas encore atteint la cible
    mais qu'on prend le shotgun, c'est les dommages du shotgun qui seront appliques !
    Il faut creer les Bullets avec leur damage !
'''
import pygame as pg
import sys
from os import path
...
class Game:
    ...
    def load_data(self):
        ...
        self.map_folder = path.join(game_folder, 'maps')  # part22/4: self
       
        # chargement de la nouvelle map
        #self.map = Map(path.join(game_folder, 'map3.txt'))
        # part22/1: bug1: deplacement de ces lignes dans new()
        ...
        # part22/5: police zone d'affichage du nb de zombies restants
    ...
    def new(self):
        ...
        # part22/2: correction bug1 (TESTER)
        self.map = TiledMap(path.join(self.map_folder, 'level1.tmx'))  # part22/3: self.map_folder
        self.map_img = self.map.make_map()
        self.map_rect = self.map_img.get_rect()        
    ...
    def update(self):
        ...
        for hit in hits: # hit est un zombie (mobile)
            ...
            # part22/13: gere les damages du Bullet
    ...
    def draw(self):
        ...
        # part22/6: affiche le nb de zombies restants (TESTER)
