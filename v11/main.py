'''
Implemente un éditeur de carte plus élaboré que le fichier map.txt
part11/1: d'abord, ajouter des mobiles sur la carte map3.txt
part11/2: télécharger et installer le pgm Tiled (mapeditor.org)
part11/3: avec Tiled créer une carte de 50x30 tuiles de 64px de côté,
          puis charger le fichier des tuiles (pas de marge ni espacement), le nom sera "Tuiles" et le
          résultat sera le fichier "Tuiles.tsx"
          Puis choisir l'outil de remplissage et tout peindre avec la tuile verte (herbe).
          Mais c'est moche car on a utilisé une seule tuile. Annuler le remplissage, 
          puis sélectionner les 4 tuiles d'herbe verte, cliquer sur les "dés" (mode aléatoire)
          et remplir à nouveau.
          Changer le nom du layer actuel en "ground".
          Ajouter un nouveau "calque de tuiles" appelé "Trees",
          sélectionner l'outil "Tampon et placer quelques arbres d'une tuile de large.
          Sauver la map dans un nouveau dossier "maps", le fichier s'appellera "tiled1.tmx".
          Il ne reste plus qu'à faire la 1ére "vraie" carte.
'''
import pygame as pg
import sys
from os import path
