...
class Mob(pg.sprite.Sprite):
    def __init__(self, game, x, y):
        ...
        pg.sprite.Sprite.__init__(self, self.groups)
        self.game = game
        #self.image = game.mob_img  # part18/4: bug expliqué plus bas
    ...
    def update(self):
        # part18/3: refactoring et prise en compte du DETECT_RADIUS
        #self.rot = (self.game.player.pos - self.pos).angle_to(vec(1, 0))
        target_dist = self.game.player.pos - self.pos
        #if target_dist.length() < DETECT_RADIUS:    # la racine carré, c'est lent !
        # (TESTER: il y a un bug si on tir sur le 1er zombie immobile, quand il bouge
        # il aura 2 barres de vie ainsi que tous les autres zombies)
        # c'est parce que tant qu'ils n'ont pas bougé, ils partagent la même image !
        # et que l'image initiale est modifiée quand on ajoute une barre d'état !
        if target_dist.length_squared() < DETECT_RADIUS ** 2:
            # part18/11: les zombies font du bruit de temps en temps
            if random() < 0.002:
