...
def collide_with_walls(sprite, group, dir):
    ...
    if dir == 'x':
        ...
        if hits:
            # part10/10: il faut changer le test sinon les zombies possuent le joueur dans les murs
            #if sprite.vel.x > 0:
...
class Mob(pg.sprite.Sprite):
    def __init__(self, game, x, y):
        ...
        self.pos = vec(x, y) * TILESIZE
        self.rect.center = self.pos
        self.health = MOB_HEALTH    # part10/2
    ...
    def update(self):
        ...
        # part10/3: test si le zombie n'a plus de vie
        if self.health <= 0:
            self.kill()
            
    # part10/5: affichage de la barre de vie
